...
class CodeAgentHandler(FileSystemEventHandler):
    ...
    def traiter_instruction(self, filepath):
        log_info(f"Nouvelle instruction détectée dans {INSTRUCTION_FILE}...")
        ...
        try:
            ...
            if not instruction:
                log_warn("Le fichier d'instructions est vide. Aucune action.")
                return

            log_info("Envoi de la requête à OmniRoute...")
            ...
            log_info("Réponse de l'IA reçue avec succès !")
            # (Ici nous ajouterons la logique pour écrire les fichiers)
            
        except Exception as e:
            log_error(f"Erreur lors du traitement", erreur=str(e))

def main():
    log_info(f"Agent de codage démarré. Surveillance du dossier : {WORKSPACE_DIR}")
